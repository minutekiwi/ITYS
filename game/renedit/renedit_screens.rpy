screen input_note(prompt):
    ...
    vbox:
        ...
        add "renedit/addnotebase.png" xpos 0.5 # Icon for decoration
        label _("{color=#90a6ad}Write your note below.\n{size=18}Escape to cancel.") text_outlines [(absolute(2),renedit_blue,0,0)] # Prompt
        input id "input" color "#90a6ad" outlines [(absolute(2),renedit_blue,0,0)]# Input styling
    text "TIP: Type \"HELP\" to view Ren'Edit commands." size 28 color "#90a6ad" outlines [(absolute(2),renedit_blue,0,0)]
screen renedit_help():
    modal True
    add Solid("#181F27EB")
    ...
    vbox:
        ...
        vbox:
            ...
            hbox:
                ...
                text _("Toggle visibility of total edit counts. Currently set to [persistent.visible_log].") color "#90a6ad" outlines [(absolute(2),renedit_blue,0,0)]
            add Solid(renedit_blue,ysize=5,xsize=1000,xalign=0.5)
            hbox:

                label _("ALLOW MULTI") xsize 350
                text _("Toggle ability to log multiple voice issues per line. Currently set to [persistent.allow_multi].") color "#90a6ad" outlines [(absolute(2),renedit_blue,0,0)]
            add Solid(renedit_blue,ysize=5,xsize=1000,xalign=0.5)
...
screen renedit_overlay():
    key "e" action Hide("renedit_overlay")
    hbox:
        vbox:
            fixed: ### We set the add note button as a fixed so we can display the note count correctly.
                ...
                imagebutton: ### ADD_NOTE BUTTON
                    idle "addnotebase" hover "addnotebase" hover_foreground "addnotehover" # Imagebutton appearance
                    action Function(add_note) # What it does
                    tooltip _("{size=18}Add a note on the current line.\nUse this for any notes for us.") # A tooltip!
                ...
                if (renpy.get_filename_line(),"note") in persistent.issue_log:
                    $ note_count = persistent.issue_log.count((renpy.get_filename_line(),"note"))
                    frame:
                        background None
            ...
            if (renpy.get_filename_line(),"issue") in persistent.issue_log and not persistent.allow_multi:
                imagebutton:
                    idle "logtypobase" hover "logtypobase" # Imagebutton appearance
                    tooltip _("{size=18}Issue logged at current line. Thanks!") # A tooltip!
            ...
            ## If it DOESN'T EXIST, we show the button.
                ...
                fixed:
                    ...
                    imagebutton:
                        idle "logtypobase" hover "logtypobase" hover_foreground "logtypohover" # Imagebutton appearance
                        action Function(add_typo) # What it does
                        tooltip _("{size=18}Log a voicing problem on the current line.\n(Missing lines, wrong lines, etc. Use the note function to add detail, or anything else.)") # A tooltip!
                        hovered SetVariable("renedit_color",renedit_green) # Set the tooltip background color to match the icon.
                    if persistent.allow_multi:
                        if (renpy.get_filename_line(),"typo") in persistent.issue_log:
                            $ typo_count = persistent.issue_log.count((renpy.get_filename_line(),"typo"))
                            frame:
                                background None
                                yalign 0.95 xalign 1.0
    ...
    if persistent.visible_log:
        vbox:
            xalign 1.0
            frame:
                xalign 1.0 xsize 350
                text str(persistent.total_note_count) + " notes created." xalign 1.0 text_align 1.0
                background Frame(renedit_blue)
            frame:
                xalign 1.0 xsize 350
                text str(persistent.total_typo_count) + " typos found." xalign 1.0 text_align 1.0
                background Frame(renedit_green)
