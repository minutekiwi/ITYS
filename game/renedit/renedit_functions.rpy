default vi = ""
default vainfo = "vainfo"
init python:

    ### Function to call the writing of the note in a new context.
    def write_the_note():
        global note
        note = renpy.input("what's up?",screen="input_note")
        return note

    ### Log a typo to the file.
    def add_typo():
        vi = _get_voice_info()
        if _last_say_who != None:
            with open( os.path.join( renpy.config.gamedir, "renedit/EditingLog.txt" ), 'a+' ) as editlog:

                vainfo = "\n".join( [ "{}={}".format(k, getattr(vi, k))
                                      for k in dir(vi)
                                      if k not in dir(object) ] )
                editlog.write("\nIssue logged at "+ str(renpy.get_filename_line()) +"\n" + vainfo + ":\n" + _last_say_who +": "+ _last_say_what + "\n\n----------------------------\n")
            editlog.close()
            issue_log_line = renpy.get_filename_line()
            persistent.issue_log.append((issue_log_line,"issue"))
            persistent.total_typo_count += 1
            renpy.notify("Successfully logged issue to file.")
        else:
            renpy.notify("No voice info found. Please use the note function instead.")
    ...
    def add_note():
        ...
        elif note == "CLEAR DATA": ## Let the user clear data.
...
label confirm_deletion:
    ...
    menu:
        ...
        "Yes, delete all persistent issue/note data. (This will not effect EditingLog.txt)":
